...
public class MaarExamplelaurencemaarCheckStage {
    ...
    //@Test
    @ParameterizedTest
    @CsvFileSource(resources = "/MaarExampleCheckStage.csv")
    /*
    @CsvSource(
            {
                    "AWNFH7FGH5"
    ...
    */
    public void selenium_01_MaarExample_CheckStage(String CaseID)
    {
        System.out.printf("========================================\n");
        ...
        try {
            ...
            if(Sleepy) Thread.sleep(500); // Short pause so that a human viewer can validate the script is working

            if(
                // SQL : IN ('RISK REVIEW','UNVERIFIED','VERIFYING','VERIFIED','AUTHENTICATION CHECK OUT HOLD')
                CaseStage.equals("CPL GENERATED") ||
            ...
            // Find Processing
            if( driver.findElements(By.xpath("//span[contains(text(),'Processing')]")).size() > 0 )
            {
                System.out.println("TEST :: Case " + CaseID + " = " + ANSI_RED + "Task stuck in PROCESSING" + ANSI_RESET);
